...
export async function GET(req: NextRequest) {
  ...
  if (!config.cronSecret) {
    if (isProd) {
      console.error(
        "[cron/detect-campaigns] CRON_SECRET is not set in production — refusing to run this endpoint unauthenticated. Set CRON_SECRET in Vercel."
      );
      return NextResponse.json({ error: "Cron not configured." }, { status: 503 });
    }
    console.warn("[cron/detect-campaigns] CRON_SECRET unset — running WITHOUT auth (non-production only).");
  } else {
