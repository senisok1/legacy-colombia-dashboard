...
//
// SCOPING, deliberately: bookings and guests both come from the existing
// property-scoped fetchers, so a search can only ever return records for
// the property currently selected. Page results are filtered through
...
export async function GET(req: NextRequest) {
  ...
  const [bookings, guests] = await Promise.all([
    getBookings(session.organizationId, groupId).catch(() => []),
