...
import { deleteConstructionActivityLogEntry, isConstructionOwner } from "@/lib/construction";

export const dynamic = "force-dynamic";

// Deleting a Construction Management activity-log entry (2026-08-20, Seni's
// ask: "only allow me, Seni Sok, to delete the activity logs"). Split out
// from api/construction/route.ts's DELETE (which removes a checklist item)
// since these are two different tables/actions — same reasoning as
// api/management/activities being its own route from api/management.
// Already covered by src/proxy.ts's CONSTRUCTION-role allowlist (it matches
// on the "/api/construction" prefix), though in practice only Seni's own
// CEO session will ever pass the check below.
