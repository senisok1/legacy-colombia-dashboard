...
export default async function RevenueManagementPage() {
    ...
    return (
      <div className="mx-auto max-w-6xl px-6 py-6">
        <p className="text-sm text-black/50 dark:text-white/50">
          Database isn&rsquo;t connected yet, so Revenue Management isn&rsquo;t available.
        </p>
      </div>
    );
  }
  ...
  const [snapshots, overrides] = await Promise.all([getLatestRateSnapshots(orgId), getLatestRateOverrides(orgId)]);
